...
# get ignition time
ignition_info <- read.csv(paste(results_output_link, "/csvs/fire_emissions.csv", sep = "")) %>% 
  ...
  arrange(date_time) %>% 
  slice(1) %>%
  mutate(igntition_time = ifelse(hour(date_time) <=9, 
                                 as.character(str_pad(hour(date_time), width = 4, side = "both", pad = 0)),
                                 as.character(str_pad(hour(date_time), width = 4, side = "right", pad = 0))))
...
This report documents the recent air quality, smoke analysis and weather forecast for the `r params$BURN_NAME` prescribed fire on the `r params$FOREST`, planned for `r burn_info$date` (`r burn_info$acres` acres with an ignition time of `r burn_info$ignition_time`). Additional smoke modeling output can be found at the [BlueSky Playground Dispersion Results](`r bsky_link`). For more information: `r contact_info`.
